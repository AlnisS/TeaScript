String[] boolfs = {"isEmpty", "equals", "equalsic", "startsWith", "endsWith", "bar"};
...
boolean doBoolf(String exp) {
  ...
  switch(sp[0]) {
    ...
    case "bar":       return getBArr(sp[1]).get(sint(feval(sp[2])));
    default: error("NOCOMMAND", "no boolean command: "+exp);
  }
  return false;
}

String[] strfs = {"substring", "lowercase", "uppercase", "trim", "trimPar", "smartTrim", "sar"};
...
String doStrf(String exp) {
  ...
  switch(sp[0]) {
    ...
    case "sar":       return getSArr(sp[1]).get(sint(feval(sp[2])));
...
float doMath(String exp) {
  ...
  switch(sp[0]) {
    ...
    case "length":   if(isString(sp[1])) return streval(sp[1]).length();
                     if(isFArr(smartTrim(sp[1]))) return getFArr(smartTrim(sp[1])).size();
                     if(isSArr(smartTrim(sp[1]))) return getSArr(smartTrim(sp[1])).size();
                     if(isBArr(smartTrim(sp[1]))) return getBArr(smartTrim(sp[1])).size();
    ...
    case "far":       return getFArr(sp[1]).get(sint(feval(sp[2])));
